    ...
    <div className="fixed top-0 left-0 right-0 z-50 pt-4 px-4 sm:px-8 md:px-12 lg:px-16 xl:px-20 pointer-events-none">
      <motion.header
        ...
        className={cn(
          ...
          'h-16 sm:h-20 flex items-center px-4 sm:px-6 lg:px-8',
        )}
      >
        <div className="flex-1 flex items-center justify-between gap-3 lg:gap-6">
                            ...
                            'text-sm font-medium transition-colors hover:text-primary whitespace-nowrap bg-transparent',
                            {
                              'text-primary': (item.subItems as Record<string, unknown>[])?.some(
                                (si: Record<string, unknown>) =>
                                  ...
                                  pathname.includes(
                                    (si.link as Record<string, unknown>).url as string,
                                  ),
                              ),
                            },
                          )}
                          ...
                          className={cn(
                            'text-sm font-medium transition-colors hover:text-primary whitespace-nowrap px-4 py-2 rounded-md hover:bg-accent/50',
                            {
                              'text-primary':
                                ...
                                (item.link as Record<string, unknown>).url !== '/'
                                  ? pathname.includes(
                                      (item.link as Record<string, unknown>).url as string,
                                    )
                                  : false,
                            },
                          )}
